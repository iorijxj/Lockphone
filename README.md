...
- **家长设置**（PIN 保护，二级密码确认）：
  ...
  - 锁定竖屏（关闭自动旋转）
  - 锁定音量（仅媒体音量：连蓝牙音响时限 70%–100%，未连时锁定 70%；不影响铃声/闹钟）
  - 密码错误记录（记录每次输错 PIN 的日期时间，可查看总次数）
- **限额提示**：白名单 APP 因系统限时打不开时，弹出「该应用今日可能已达使用限额」提示（启发式检测）
- **开机自启**：重启后自动回到锁定桌面（需在系统「自启动管理」中授权）
- **时间管理**：交给系统自带「应用限时」承担，本 APP 不自行计时
...
- **限额提示**：OriginOS 的限时状态无法从系统读取，提示为启发式（点击后约 1.5 秒未进入前台即判定），不够精确
